...
class BertModel():
    ...
    def forward(self,):
        ...
        for i in myPath:#执行每一个测试集
            num,maxlen=_getvec(i,False,num,maxlen)

        #执行训练集
        num,maxlen=_getvec(pathVal,True,0,maxlen)
        return maxlen
        
    def _getVec(self, path, flag, num, maxlen):
    ...
    def _getFeature(text):#根据Bert模型的特征，提取文字的embeding
        text = tokenizer.tokenize(text)
        text_id = tokenizer.convert_tokens_to_ids(text)
        text_id = torch.tensor(text_id, dtype=torch.long)
        text_id = text_id.unsqueeze(dim=0)
